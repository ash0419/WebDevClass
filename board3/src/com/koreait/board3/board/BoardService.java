...
public class BoardService {
	...
	public static String regMod(HttpServletRequest request) {
		...
		if (i_board == 0) { // 등록
			String sql = " INSERT INTO t_board " + " (typ, seq, title, ctnt, i_user) " + " SELECT "
					+ " ?, ifnull(max(seq), 0) + 1, ?, ?, ? " + " FROM t_board " + " WHERE typ = ? ";
...
//		return BoardDAO.selBoard(p);
	...
	public static String ajaxFavorite(HttpServletRequest request) {
		...
		case 0: // 좋아요 해제
			...
			break;
		case 1: // 좋아요 처리
			sql = " INSERT INTO t_board_favorite (i_board, i_user) VALUES (?, ?) ";
			break;
		}
